...
\lstinputlisting[
  style=cpp,
  caption={Template for the \textbf{Point} structure (or 2D vector) that has its own subtraction operator and its cross product},
  label=lst:cross-product
]{code/maths/cross-product.cpp}

\subsubsection{Polygon Area}

\lstinputlisting[
  style=cpp,
  caption={Obtains the area of a polygon formed by a set of Points (2D) stored in a \texttt{std::vector} with a time complexity of \texttt{O(n)}. It returns a double, since it has a division by 2, but it can return a \texttt{ll} for double the area. Requires the \texttt{Point} struct seen in the previous listing.},
  label=lst:polygon-area
]{code/maths/polygon-area.cpp}
